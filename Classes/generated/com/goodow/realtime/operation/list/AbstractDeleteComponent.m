...
- (id<ComGoodowRealtimeOperationOperation>)transformWithComGoodowRealtimeOperationOperation:(id<ComGoodowRealtimeOperationOperation>)other
  ...
  switch (op->type_) {
    ...
    if (op->startIndex_ <= startIndex_) {
      return [self canUndo] ? [self createWithInt:startIndex_ + op->length_ withId:values_] : [self createWithInt:startIndex_ + op->length_ withInt:length_];
    }
    ...
    if (endIndex1 <= startIndex_) {
      ...
      return endIndex1 < endIndex0 ? ([self canUndo] ? [self createWithInt:op->startIndex_ withId:[((id<ComGoodowRealtimeOperationListListHelper>) nil_chk([self getHelper])) subsetWithId:values_ withInt:endIndex1 - startIndex_ withInt:len]] : [self createWithInt:op->startIndex_ withInt:len]) : nil;
    }
    else {
      if (endIndex1 < endIndex0) {
        return [self canUndo] ? [self createWithInt:startIndex_ withId:[((id<ComGoodowRealtimeOperationListListHelper>) nil_chk([self getHelper])) replaceWithWithId:values_ withInt:op->startIndex_ - startIndex_ withInt:op->length_ withId:nil]] : [self createWithInt:startIndex_ withInt:length_ - op->length_];
      }
      else {
        return [self canUndo] ? [self createWithInt:startIndex_ withId:[((id<ComGoodowRealtimeOperationListListHelper>) nil_chk([self getHelper])) subsetWithId:values_ withInt:0 withInt:op->startIndex_ - startIndex_]] : [self createWithInt:startIndex_ withInt:op->startIndex_ - startIndex_];
      }
    }
    case ComGoodowRealtimeOperationListAbstractReplaceComponent_TYPE:
    if (![self canUndo] || endIndex1 <= startIndex_ || op->startIndex_ >= endIndex0) {
      return self;
    }
    else if (op->startIndex_ <= startIndex_) {
      if (endIndex1 < endIndex0) {
...
- (IOSObjectArray *)transformComponentWithComGoodowRealtimeOperationOperationComponent:(ComGoodowRealtimeOperationOperationComponent *)other
  ...
  ComGoodowRealtimeOperationListAbstractListComponent *op = (ComGoodowRealtimeOperationListAbstractListComponent *) check_class_cast(other, [ComGoodowRealtimeOperationListAbstractListComponent class]);
  int endIndex0 = startIndex_ + length_;
  NSAssert(((ComGoodowRealtimeOperationListAbstractListComponent *) nil_chk(op))->type_ == ComGoodowRealtimeOperationListAbstractInsertComponent_TYPE && op->startIndex_ > startIndex_ && op->startIndex_ < endIndex0, @"/Users/retechretech/dev/workspace/realtime/realtime-operation/src/main/java/com/goodow/realtime/operation/list/AbstractDeleteComponent.java:122 condition failed: assert op.type == AbstractInsertComponent.TYPE && op.startIndex > startIndex\n        && op.startIndex < endIndex0;");
  int len0 = op->startIndex_ - startIndex_;
  int len1 = length_ - len0;
  return [self asArrayWithNSObjectArray:[IOSObjectArray arrayWithObjects:(id[]){ [self canUndo] ? [self createWithInt:startIndex_ withId:[((id<ComGoodowRealtimeOperationListListHelper>) nil_chk([self getHelper])) subsetWithId:values_ withInt:0 withInt:len0]] : [self createWithInt:startIndex_ withInt:len0], [self canUndo] ? [self createWithInt:startIndex_ + op->length_ withId:[((id<ComGoodowRealtimeOperationListListHelper>) nil_chk([self getHelper])) subsetWithId:values_ withInt:len0 withInt:len1]] : [self createWithInt:startIndex_ + op->length_ withInt:len1] } count:2 type:[IOSClass classWithClass:[NSObject class]]]];
}
...
+ (J2ObjcClassInfo *)__metadata {
  ...
  static J2ObjcClassInfo _ComGoodowRealtimeOperationListAbstractDeleteComponent = { "AbstractDeleteComponent", "com.goodow.realtime.operation.list", NULL, 0x401, 9, methods, 1, fields, 1, superclass_type_args};
