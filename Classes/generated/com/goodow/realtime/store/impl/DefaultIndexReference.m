...
#include "com/goodow/realtime/store/impl/DefaultReferenceShiftedEvent.h"

@implementation ComGoodowRealtimeStoreImplDefaultIndexReference
...
- (id)referencedObject {
  return [((ComGoodowRealtimeStoreImplDefaultModel *) nil_chk(model_)) getObjectWithNSString:referencedObjectId_];
}
...
withComGoodowRealtimeOperationOperationComponent:(ComGoodowRealtimeOperationOperationComponent *)component {
  ...
  NSAssert(((ComGoodowRealtimeOperationCursorReferenceShiftedComponent *) nil_chk(op))->oldIndex_ == [self index], @"/Users/retechretech/dev/workspace/realtime/realtime-store/src/main/java/com/goodow/realtime/store/impl/DefaultIndexReference.java:77 condition failed: assert op.oldIndex == index();");
  referencedObjectId_ = op->referencedObjectId_;
  index__ = op->newIndex_;
  canBeDeleted__ = op->canBeDeleted_;
  if (op->oldIndex_ != -1) {
    id<ComGoodowRealtimeStoreReferenceShiftedEvent> event = [[ComGoodowRealtimeStoreImplDefaultReferenceShiftedEvent alloc] initWithComGoodowRealtimeJsonJsonObject:[((id<ComGoodowRealtimeJsonJsonObject>) nil_chk([((id<ComGoodowRealtimeJsonJsonObject>) nil_chk([self eventWithNSString:sessionId withNSString:userId])) setWithNSString:@"oldIndex" withDouble:op->oldIndex_])) setWithNSString:@"newIndex" withDouble:op->newIndex_]];
...
+ (J2ObjcClassInfo *)__metadata {
  static J2ObjcMethodInfo methods[] = {
    { "initWithComGoodowRealtimeStoreImplDefaultModel:", "DefaultIndexReference", NULL, 0x0, NULL },
  ...
  static J2ObjcClassInfo _ComGoodowRealtimeStoreImplDefaultIndexReference = { "DefaultIndexReference", "com.goodow.realtime.store.impl", NULL, 0x0, 10, methods, 3, fields, 0, NULL};
  return &_ComGoodowRealtimeStoreImplDefaultIndexReference;
}
